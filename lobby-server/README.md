...
from source
```bash
git clone https://github.com/author_name/project_urlname lobby_server
cd lobby_server
make install
```
...
```bash
pip install lobby_server
```

## Executing

```bash
$ lobby_server run --port 8080
```

or

```bash
python -m lobby_server run --port 8080
```

or

```bash
$ uvicorn lobby_server:app
```
...
```bash
❯ lobby_server --help
Usage: lobby_server [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ lobby_server create-user --help
Usage: lobby_server create-user [OPTIONS] USERNAME PASSWORD
...
```bash
lobby_server create-user admin admin --superuser
```
...
```py
from lobby_server.config import settings
```
...
```bash
PROJECT_NAME_ENV=production lobby_server run
```
